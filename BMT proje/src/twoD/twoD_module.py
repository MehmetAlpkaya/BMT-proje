...
class CtwoD(QDialog):
    ...
    def gray_clicked(self):
        try:
            ...
            QMessageBox.about(None, 'Bilgi', 'Zaten gri tonlamaya dönüştürülmüş!')
        self.imgLabel_1.display_image()
        self.imgLabel_2.display_image()
    ...
    def draw_clicked(self, status):
        if status:
            ...
            self.drawButton.setText('Çizmeye başla')
        else:
            self.imgLabel_2.drawornot = False
            self.drawButton.setText('Çizimi durdur')
    ...
    def load_clicked(self):
        fname, _filter = QFileDialog.getOpenFileName(self, 'open file', '~/alpka/OneDrive/Belgeler',
